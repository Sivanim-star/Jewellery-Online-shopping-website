...
function loadOrderHistory() {
    const saved = localStorage.getItem('luxeOrderHistory');
    if (saved) {
...
function saveOrderToHistory(orderData) {
    ...
    localStorage.setItem('luxeOrderHistory', JSON.stringify(orderHistory));
}
        // Initialize
...
function decreaseQuantity(index) {
    ...
    updateCart();
        ...
        function placeOrder() {
            // Validate form
            // Inside placeOrder function, before cart = [];
...
function showOrderHistory() {
    ...
    if (!container) {
        // Need to add order history section to HTML
        return;
    }
        ...
        <div class="order-history-card">
            <div class="order-header">
                <div>
                    ...
                    <div class="order-date">${new Date(order.date).toLocaleDateString('en-IN', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric'
                    })}</div>
                ...
                ${order.items.map(item => `
                    <div class="order-item">
                        <div class="order-item-details">
                            <div style="font-weight: 600;">${item.name}${item.quantity > 1 ? ` x${item.quantity}` : ''}</div>
                            <div style="color: var(--grey-light); font-size: 0.9rem;">${item.metal} • ${item.carat}</div>
                        </div>
                        <div style="color: var(--gold); font-weight: 600;">₹${(item.price * item.quantity).toLocaleString()}</div>
                    </div>
                `).join('')}
            </div>
            
            <div style="display: flex; justify-content: space-between; padding-top: 1rem; border-top: 1px solid rgba(212, 175, 55, 0.1);">
                <span style="color: var(--grey-light);">Total Amount:</span>
                <span style="color: var(--gold); font-size: 1.3rem; font-weight: 700;">₹${order.total.toLocaleString()}</span>
            </div>
            
            <div style="margin-top: 1rem; color: var(--grey-light); font-size: 0.9rem;">
                <strong>Delivery:</strong> ${order.deliveryDate}<br>
